...
crates/
  ...
  duels-value/          a LEARNED position value (feasibility spike): public-information features,
                        a small network trained offline on actual game outcomes as a 4-way
                        victory-kind-decomposed classifier, hand-rolled f32 inference, weights
                        embedded. Same layer as duels-eval, depends on duels-core only. Consumed
                        by mcts-eval through opt-in LeafValue::Learned / LearnedBlend, NOT a
                        default anywhere — read its crate docs for what it measured
...
This project has built and refined eight agents plus a strategy layer, four of which have since been retired. A consistent discipline emerged; follow it for any new agent work:
...
- **A *learned* static value is not a better leaf than the hand-crafted one, even when it is a much better predictor.** `duels-value` (a 493-128-4 network trained on the actual outcomes of the 100k-game corpus) is far better calibrated on held-out games than `duels_eval::win_probability` (Brier 0.183 vs 0.217, three-quarters of the way to the search's own 2,000-simulation verdict), and yet: alone as a leaf it is `-172` Elo at `Nodes(32000)` — the same number, and the same over-collects-science signature, as the pure `duels-eval` leaf — and blended half-and-half with a playout it is at *parity* with the default blend (`+21.7` Elo over 400 games, CI containing zero). The victory-kind decomposition of the training target did help the aggregate calibration (measured against a single-scalar control), and its `science_win` head visibly changes *how* the search wins (25 science wins to 0), but not how often. Read `duels-value`'s crate docs before spending more on this line; the leading untested hypothesis is the distribution shift between self-play trajectories and positions inside a search tree.
