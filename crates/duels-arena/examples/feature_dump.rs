...
//! ```
//!
//! then `rows` records of `12 + 4 * num_features` bytes:
//!
//! ```text
...
/// Bytes per record: seed, label, search value, then the features.
const RECORD_BYTES: usize = 12 + 4 * NUM_FEATURES;
        ...
        if labelled {
            ...
            if kept.is_multiple_of(stride) {
                for me in Player::ALL {
                    ...
                    // The corpus stores the root value from Player One's view;
                    // this row's perspective may be the other one.
                    let value = match me {
                        Player::One => d.value,
                        Player::Two => 1.0 - d.value,
                    };
                    write_record(&mut buf, g.seed, label, value as f32, &features(&state, me));
                }
            }
            kept += 1;
        }
...
fn write_record(buf: &mut Vec<u8>, seed: u64, label: Outcome, value: f32, f: &[f32; NUM_FEATURES]) {
