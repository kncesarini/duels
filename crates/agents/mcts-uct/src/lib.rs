...
//! **explicit chance nodes**, whose leaf value is always a **playout to a
//! real [`duels_core::GameResult`]**.
//!
//! # This crate is deliberately playout-only
//!
//! A leaf value that mixes `duels-eval`'s hand-crafted evaluation into the
//! playout measures `+89.2` Elo over 3,600 games against this agent, and it
//! lives in its own crate, `duels-agent-mcts-eval`, rather than as a
//! `Config::leaf` option here.
//!
//! That split is the point, not an accident of history. The winning
//! configuration rescales [`Config::exploration`] — a blended reward has half
//! the spread a Bernoulli playout does, so `c` has to be halved with it — and
//! **every knob in this crate's [`Config`] was tuned against `c = 1.0`**: the
//! rollout weights, the [`RaceWeights`] tables, the widening constants, the
//! [`PriorMode`] sweep. Carrying both settings in one `Config` would have
//! meant every future measurement here having to say which `c` it was taken
//! at. So this crate stayed exactly what it was, `c = 1.0` and a playout
