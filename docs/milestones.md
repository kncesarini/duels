...
| **M5** RL pipeline | PyO3 bindings, self-play, ONNX, training loop, `mcts-valuenet`/`mcts-nn` | 🚧 Underway in a different shape than originally scoped — `duels-value`/`mcts-value` are the bridge step and have now run the loop with real exploration and a generations registry (`v1` → `v2` → `v3` → `v4`, `crates/duels-value/weights/generations.json`); Generation 3 (`v4`) landed a real but much smaller gain than `v2` → `v3` (+17.2 Elo vs +61.0/+53.9), close to the loop's own ±10 Elo plateau stop-rule — `docs/roadmap.md`'s "Generation 3" section has the honest read and is the live plan for iterating further |
