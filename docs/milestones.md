...
| **M5** RL pipeline | PyO3 bindings, self-play, ONNX, training loop, `mcts-valuenet`/`mcts-nn` | 🚧 Underway in a different shape than originally scoped — `duels-value`/`mcts-value` are the bridge step and have now run the loop with real exploration and a generations registry (`v1` → `v2` → `v3`, `crates/duels-value/weights/generations.json`); Generation 3 (`tier1-gen3-l05`) landed a real but much smaller gain than `v2` → `v3` (+17.2 Elo vs +61.0/+53.9) and was **held, not promoted**, after a frozen-panel non-regression check found it read weaker than `v3` against two of three non-ancestor panel members — `docs/roadmap.md`'s "Generation 3" and "autonomous loop" sections have the honest read and the live plan for a properly-gated next attempt |
