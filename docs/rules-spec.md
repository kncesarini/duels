...
| R-038 | A cost can be itemised per resource (`cost::PaymentPlan`): every printed unit is accounted for exactly once as produced, covered by a choice source, covered by a rebate, or bought at that resource's current unit price, and the bought units always sum back to the same coins `card_cost` / `wonder_cost` charge — for either player, whether or not it is their turn. The itemisation is derived from the same greedy assignment the cost itself comes from, so it cannot drift. | `properties::check_payment_plan` (asserted at every decision point of `invariants_hold_throughout_a_game`) |
